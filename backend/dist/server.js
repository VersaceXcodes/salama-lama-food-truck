        ...
        await with_client(async (client) => {
         ...
         VALUES ($1,$2,$3,$4,$5,$6)`, [gen_id('osh'), order_id, 'received', req.user.user_id, created_at, 'Order placed']);
            ...
            if (totals.discount_code) {
             ...
             VALUES ($1,$2,$3,$4,$5,$6)`, [gen_id('du'), dc.code_id, req.user.user_id, order_id, totals.discount_amount, now_iso()]);
             ...
             WHERE reward_code = $3 AND loyalty_account_id = (SELECT loyalty_account_id FROM loyalty_accounts WHERE user_id = $4)
               AND usage_status = 'unused'`, [order_id, now_iso(), ensure_upper(totals.discount_code), req.user.user_id]);
                }
            }
...
app.put('/api/admin/customers/:id/points', authenticate_token, require_role(['admin']), async (req, res) => {
    try {
        ...
        await with_client(async (client) => {
          ...
          transaction_id, loyalty_account_id, transaction_type, points_amount,
          order_id, reason, adjusted_by_user_id, running_balance, created_at, expires_at
        ) VALUES ($1,$2,'manual_adjustment',$3,NULL,$4,$5,$6,NULL)`, [gen_id('pt'), la.loyalty_account_id, delta, body.reason, req.user.user_id, next_balance, now_iso()]);
            await client.query('COMMIT');
