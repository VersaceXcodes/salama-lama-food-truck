...
describe('Admin Invoices - Null user_id handling', () => {
  it('GET /api/admin/invoices should return 200 with invoices including null user_id', async () => {
    ...
    const nullUserInvoice = response.body.invoices.find(
      (inv: { invoice_id: string }) => inv.invoice_id === 'inv_null_user_001'
    );
    ...
    const withUserInvoice = response.body.invoices.find(
      (inv: { invoice_id: string }) => inv.invoice_id === 'inv_with_user_001'
    );
