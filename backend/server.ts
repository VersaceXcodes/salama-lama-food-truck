...
function read_cart_sync(user_id) {
  ...
  if (!fs.existsSync(fp)) return { items: [], discount_code: null, updated_at: now_iso() };
  try {
    const parsed = JSON.parse(fs.readFileSync(fp, 'utf8'));
    if (!parsed || typeof parsed !== 'object') return { items: [], discount_code: null, updated_at: now_iso() };
    ...
    if (selected && typeof selected === 'object') {
      for (const v of Object.values(selected)) {
        if (typeof v === 'string') {
          ...
          for (const sub of v) {
            if (typeof sub === 'string') option_ids.push(sub);
          }
        }
      }
    }
