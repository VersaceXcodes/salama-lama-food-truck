...
app.get('/api/orders/:id/track', authenticate_token, async (req, res) => {
  try {
    ...
    const order_res = await pool.query(
      'SELECT order_id, order_number, status, order_type, collection_time_slot, delivery_address_snapshot, estimated_delivery_time, customer_name, customer_phone FROM orders WHERE order_id = $1 AND user_id = $2',
